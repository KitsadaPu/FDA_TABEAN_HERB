        ...
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    @*<th ng-click="sortColumn('Expdate_Before_Renew')">วันที่หมดอายุ</th>*@
                    <th ng-click="sortColumn('rcv_StaffName')">เจ้าหน้าที่รับเรื่อง</th>
                    <th>รายละเอียด</th>
                </tr>
            </thead>
            <tbody>
                <tr ng-if="Renew_Tabean_Staff.length===0">
                    <td colspan="10" class="text-center text-base-danger">--- ไม่มีคำขอในระบบ ---</td>
                </tr>
                @*<tr ng-repeat="x in FULL_MODEL.Renew_Tabean_Staff | orderBy:'-TR_ID'">*@
                @* <tr dir-paginate="x in filteredData = (Renew_Tabean_Staff | orderBy:'-TR_ID' | filter:search |filter:{PROCESS_ID: FULL_MODEL.PROCESS_ID} | filter:{STATUS_ID: statusFilter} | filter:yearFilterFunction) | itemsPerPage: 20"> *@
                <tr dir-paginate="x in filteredData = (Renew_Tabean_Staff | filter:search|filter:{PROCESS_ID: FULL_MODEL.PROCESS_ID} | filter:{STATUS_ID: statusFilter} | filter:yearFilterFunction)|itemsPerPage: 20">
                    ...
                    @*<td>{{x.Expdate_Before_Renew}} </td>*@
                    <td>{{x.STAFF_NAME_RCV}} </td>
