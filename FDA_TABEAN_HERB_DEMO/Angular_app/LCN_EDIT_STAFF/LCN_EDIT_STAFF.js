...
app.controller('LCN_EDIT_STAFF', function ($scope, Upload, CENTER_SV) {
    $scope.upload_id = '';
    ...
    function pageload() {
        $scope.pick_v = 0;
           /* var date_now = new Date(Date.now()).toLocaleString("th-TH", { timeZone: "UTC" });*/
            var send_data = {
                FUNC_CODE: 'FUNC-GET_DATA_LCN_STAFF',
            ...
            getDATA.then(function (datas) {
                $scope.lcn_edit_staff = datas.data.lcn_edit_staff;
    ...
    $scope.BTN_STAFF_MENU_CLICK = function (IDA, STATUS_ID, IDA_LCN, TR_ID) {

    };

    $scope.BTN_CONFIEM_CLICK = function (IDA) {

    };

    $scope.BTN_SAVE_DATA_EDITFILE_CLICK = function (IDA) {
        var send_data = {
            FUNC_CODE: 'FUNC-CER_LCN_KEEP_PAY',
            IDA_LCN: $scope.IDA_LCN,
            IDA: $scope.IDA,
            CER_HERB_LCN: $scope.FULL_MODEL.CER_HERB_LCN,
            STATUS_ID: STATUS_ID,
        };
    ...
    $scope.BTN_KEEP_PAY_CLICK = function (IDA, STATUS_ID) {
        ...
        getDATA.then(function (datas) {
            ...
            //$scope.cer_lcn_staff = datas.data.cer_lcn_staff;
    ...
    $scope.LCN_SAVE_DATA_STAFF_CLICK = function () {

        var send_data = {
            FUNC_CODE: 'FUNC-CER_DALCN_HERB_ADD_STAFF',
            IDA_LCN: $scope.IDA_LCN,
            IDA: $scope.IDA,
            CER_HERB_LCN: $scope.FULL_MODEL.CER_HERB_LCN,
            MAS_CRITERIA_USE_IN_AUDIT_CER: $scope.MAS_CRITERIA_USE_IN_AUDIT_CER,
            MAS_PRODUCTION_MODEL_CER: $scope.MAS_PRODUCTION_MODEL_CER,
        };
    ...
    $scope.select_languege_cer = function (GROUP_ID) {
        //$('#myModal_lcn_staff').modal('show');
        //$scope.SET_MAIN_POPUP_PAGE = '../CER_HERB/cer_lcn_detail_staff'
        $scope.IDA = $scope.IDA
        //IDA_LCN: $scope.IDA_LCN,
        $scope.STATUS_ID = $scope.STATUS_ID
        var path;
        $scope.path = 'Certificate_1.pdf'
        $scope.PDF_VIEW = "../PDF/PDF_VIEW.aspx?IDA=" + IDA + "&TR_ID=" + TR_ID + "&GROUP_ID=" + GROUP_ID
    };


    $scope.BTN_STAFF_LANGUEGE1_CLICK = function (IDA, STATUS_ID, IDA_LCN, TR_ID, GROUP_ID) {
        $scope.IDA = $scope.IDA
        $scope.STATUS_ID = STATUS_ID
        window.open("../PDF/PDF_VIEW_CER.aspx?IDA=" + IDA + "&TR_ID=" + TR_ID + "&GROUP_ID=" + GROUP_ID);
    };

    $scope.BTN_STAFF_LANGUEGE2_CLICK = function (IDA, STATUS_ID, IDA_LCN, TR_ID, GROUP_ID) {
        $scope.IDA = $scope.IDA
        $scope.STATUS_ID = STATUS_ID
        window.open("../PDF/PDF_VIEW_CER.aspx?IDA=" + IDA + "&TR_ID=" + TR_ID + "&GROUP_ID=" + GROUP_ID);
    };
    ...
    //$scope.staff_name_txt = function (id) {
    //    //var req_type = $scope.DDL_STAFF_NAME.filter(x => x.IDA);
    //    //$scope.FULL_MODEL.DDL_STAFF_NAME = req_type[0].STAFF_NAME;   
    //    $scope.STAFF_NAME = id;
